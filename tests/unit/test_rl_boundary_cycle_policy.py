...
class _Topology:
    ...
    def __init__(self):
        self.outer_cycle = _Cycle({_Face((0, 1))})
        self.inner_cycle = _Cycle({_Face((0, 2)), _Face((2, 1)), _Face((1, 0))})
...
class _SensorNetwork:
    def __init__(self):
        ...
        self.mobile_sensors = [
            Sensor(np.array([0.4, 0.3]), np.array([0.1, 0.0]), 0.2, False),
        ]
...
class _Simulation:
    ...
    def step_with_velocities(self, mobile_velocities, interval=None):
        ...
        return RLStepResult(
            ...
            state_change=type("_StateChange", (), {"alpha_complex_change": lambda self: (0, 0, 0, 0), "boundary_cycle_change": lambda self: (0, 0)})(),
...
def test_boundary_cycle_policy_emits_mobile_velocity_actions():
    env = _make_env()
    obs, _ = env.reset()
