...
from rl_structured_velocity_policy import StructuredVelocityActorCriticPolicy, StructuredVelocityGraphExtractor
...
def test_structured_velocity_policy_emits_mobile_velocity_actions_and_nonnegative_beta():
    env = _make_env()
    obs, _ = env.reset()
    ...
    beta = policy.last_actor_terms["beta"].cpu().numpy()
    n_hat_mobile = policy.last_actor_terms["n_hat_mobile"].cpu().numpy()

    assert np.all(beta >= 0.0)
    assert np.linalg.norm(n_hat_mobile[0, 0]) > 0.0
    assert n_hat_mobile[0, 1] == pytest.approx(np.array([0.0, 0.0]))
