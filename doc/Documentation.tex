...
return time
\end{python}
Here we return the time taken for the evasion path to be closed. to select this option, set \pythoninline{end_time = 0}. 
...
if evasion_paths == []:
	...
	return True
\end{python}
In this case we return a boolian of wether the evasion path lasted for a given amount of time. To select this option, set \pythoninline{end_time > 0}
...
\begin{itemize}
	...
	\item \KW{Not sure how to exactly set the boundary sensor spacing}
	...
	\item \KW{Does not check if graph is connected at each timestep} (easy fix)
\end{itemize}
