...
\begin{python}
 ...
 To find holes in a given complex, we can use the following:
 \begin{python}
 from combinatorial_map import *
 # Initialize the combinatorial map
 cmap = CMap(G, points)
 
 # Initialize alpha_shape
 alpha_shape = get_boundary() # function does not exist
 
 for cycle in boundary_cycle_graphs(cmap):
 	if is_hole(cycle, alpha_shape): print(True)
 	 
 \end{python}
Note that the \pythoninline{alpha_shape} is needed as an input because \pythoninline{CMap} has no knowledge of the domain geometry. 
...
\begin{itemize}
	\item If a node moves onto the boundary it seems to "get stuck" there and this incorrectly causes the boundary to be labeled as a hole.
	\item Not sure how to exactly set the boundary sensor spacing
	\item If the a sensor gets to close to the boundary and the spacing is "large" , it distorts the boundary connectivity.
	\item The code does not currently track evasion paths
	\item No longer implements animation
\end{itemize}
