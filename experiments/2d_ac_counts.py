...
import csv
import logging
from boundary_geometry import UnitCube, UnitCubeFence, get_unitcube_fence, RectangularDomain
...
class AtomicChangeDetection:
    def __init__(self,
        ...
        self.atomic_changes = {}
        self.max_ac = max_ac
        print(f"Running atomic changes for {self.dim}D")
    ...
    def do_timestep(self, level: int = 0) -> None:
        ...
        for loop_id in range(2):
            ...
            if case == "((0, 0, 0, 0, 0, 0), (0, 0))" or case == "((0, 0, 0, 0), (0, 0))":
    ...
    def total_atomic_changes(self):
        if self.dim == 2:
            null_change = "((0, 0, 0, 0), (0, 0))"
        elif self.dim == 3:
            null_change = "((0, 0, 0, 0, 0, 0), (0, 0))"
        else:
            raise ValueError("This dimension is not implemented")
        return sum(value for key, value in self.atomic_changes.items() if key != null_change)
...
def simulate(n_sensors, radii, velocities, dt, output_file, max_atomic_changes, debug, dim=2) -> float:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_changes", type=int, default=10000, help="Maximum number of atomic changes to look for")
    parser.add_argument("--output_dir", type=str, default="./output/2d_10000/", help="Directory to save output CSV files")
    ...
    for sensing_radius in tqdm(sensing_radii):
