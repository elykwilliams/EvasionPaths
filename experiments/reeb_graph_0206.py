"""
Experiment: build a Reeb-style event graph from 2D boundary-cycle evolution.

This script runs a fresh 2D simulation and records boundary-cycle transitions
at initialization, each atomic update, and termination.
"""
...
class ReebEventGraphBuilder:
    ...
    def _edge_event(self, parent: str, child: str, children_for_parent: Dict[str, List[str]], parents_for_child: Dict[str, List[str]]) -> str:
    ...
    def add_snapshot(self, *, step: int, time: float, labels: Dict, alpha_cycle) -> None:
        ...
        # A child gets at most one dominant parent unless there is a strong tie.
        for child in born:
            ...
            # Allow true multi-parent lineage only when overlap is genuinely strong for both.
            if best_score >= 2:
        ...
        # Pre-assign lanes for born cycles so split events become visible branches.
        ...
        for parent in split_parents:
            ...
            # Keep one child on the parent lane, push siblings to fresh lanes.
            born_lane[kids[0]] = parent_lane
            for sibling in kids[1:]:
                born_lane[sibling] = self._new_lane()

        # For non-split births with lineage, inherit a stable lane when possible.
        ...
        # Explicitly terminate cycles that died with no mapped successor this step.
        # This keeps short-lived split branches visible as their own time-level endpoint.
        ...
        # Keep lane map scoped to currently active cycles to avoid stale-key collisions.
...
def run_2d_reeb_simulation(
    *,
    ...
    builder.add_snapshot(step=0, time=simulation.time, labels=simulation.cycle_label.label, alpha_cycle=simulation.topology.alpha_cycle)
    ...
    while step < max_steps:
        ...
        step += 1
        builder.add_snapshot(step=step, time=simulation.time, labels=simulation.cycle_label.label, alpha_cycle=simulation.topology.alpha_cycle)
...
def draw_reeb_graph(graph: nx.DiGraph, *, title: str = "Reeb Event Graph (2D)") -> None:
    ...
    _draw_compact_graph_panel(plt.gca(), compact, event_nodes, font_size=6)

    plt.title(title)
    plt.xlabel("time")
    plt.ylabel("cycle lane")
    plt.tight_layout()
    plt.show()
...
def animate_reeb_graph(
    ...
    *,
    ...
    def frame(step: int):
        ...
        _draw_compact_graph_panel(ax, compact, event_nodes, font_size=6)

        ax.set_ylim(-1, max(graph.nodes[n]["lane"] for n in graph.nodes) + 1)
    ...
    ani = FuncAnimation(fig, frame, frames=range(max_step + 1), interval=interval_ms, repeat=False)
    return ani
...
def _compact_view_graph(graph: nx.DiGraph) -> Tuple[nx.DiGraph, List[int]]:
    ...
    # Event nodes: branch/merge/terminate incidents, births/deaths, and label changes.
    ...
    for u, v, d in graph.edges(data=True):
        event = d.get("event")
        if event in {"split", "merge", "terminate"}:
            event_nodes.add(u)
            event_nodes.add(v)
        if d.get("label_flip"):
            event_nodes.add(u)
    ...
    def add_or_merge_edge(a: int, b: int, event: str, label_flip: bool) -> None:
        if compact.has_edge(a, b):
            ...
            # Keep explicit event label if already present.
    ...
    # Keep explicit event edges.
    for u, v, d in graph.edges(data=True):
        event = d.get("event")
        if event in {"split", "merge", "terminate"} and (u in event_nodes and v in event_nodes):
            add_or_merge_edge(u, v, event=event, label_flip=bool(d.get("label_flip", False)))

    # Compress continue/transform paths using actual directed lineage.
    for start in sorted(event_nodes, key=lambda n: (graph.nodes[n]["time"], graph.nodes[n].get("step", 0), n)):
        for succ in graph.successors(start):
            ...
            if event not in {"continue", "transform"}:
                # Non-continuation edges are handled above (or intentionally omitted).
                continue
            ...
            while current not in event_nodes:
                ...
                # Only compress through simple single-lineage continuation.
            ...
            if current in event_nodes:
                ...
                if t1 > t0:
                    add_or_merge_edge(start, current, event="continue", label_flip=accum_flip)

    event_node_list = sorted(event_nodes, key=lambda n: (graph.nodes[n]["time"], graph.nodes[n]["lane"], n))
    return compact, event_node_list


def _draw_edge_labels_safe(ax, graph: nx.DiGraph, pos: Dict[int, Tuple[float, float]], *, font_size: int = 6) -> None:
    # Labels intentionally suppressed: graph is communicated by arrows and node colors only.
    return


def _optimize_compact_layout(compact: nx.DiGraph, event_nodes: List[int], *, sweeps: int = 8) -> Dict[int, Tuple[float, float]]:
    if compact.number_of_nodes() == 0:
        return {}

    # Keep lanes stable over time: do not reorder by barycentric sweeps.
    # This avoids visual artifacts where a branch appears to "jump" lanes.
    ...
    # If multiple event nodes land on the exact same step/lane, nudge vertically.
...
def _draw_compact_graph_panel(
    ...
    *,
    ...
    _draw_edge_labels_safe(ax, graph, pos, font_size=font_size)
    ...
    *,
    ...
    for labels, alpha_change, boundary_change, time in history[1:]:
        ...
        if uncovered is None:
            # Fallback: count true labels and remove the persistent outer cycle.
...
def _make_frames_dir(user_dir: str | None = None) -> Path:
    if user_dir:
...
def _default_gif_path(prefix: str = "reeb_animation") -> Path:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return Path.cwd() / f"{prefix}_{stamp}.gif"


def _save_animation_gif(ani: FuncAnimation, path: Path, *, fps: int = 8) -> None:
...
def main() -> None:
    ...
    frames_dir = _make_frames_dir(args.frames_dir if args.frames_dir else None) if args.save_frames else None

    _simulation, builder, steps, _clear_streak = run_2d_reeb_simulation(
    ...
    print_atomic_change_report(_simulation, builder.summaries, dt=args.dt)

    if args.no_plot:
        return

    if args.coupled_animate:
        _ani = animate_sensor_and_reeb(
            builder,
            run_steps=steps,
            num_sensors=args.num_sensors,
            sensing_radius=args.radius,
        ...
        if args.save_gif:
            gif_path = Path(args.gif_path) if args.gif_path else _default_gif_path("reeb_coupled")
            _save_animation_gif(_ani, gif_path, fps=args.gif_fps)
        plt.tight_layout()
        plt.show()
    elif args.animate:
        _ani = animate_reeb_graph(builder.graph, save_frames_dir=frames_dir)
        if args.save_gif:
            gif_path = Path(args.gif_path) if args.gif_path else _default_gif_path("reeb_graph")
            _save_animation_gif(_ani, gif_path, fps=args.gif_fps)
        plt.show()
    else:
        draw_reeb_graph(builder.graph)
