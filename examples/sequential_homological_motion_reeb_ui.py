...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Interactive setup UI + post-run Reeb playback for Sequential Homological Motion."
    )
    ...
    from motion_model import SequentialHomologicalMotion
    from _motion_reeb_setup_ui import launch_motion_reeb_setup_ui

    launch_motion_reeb_setup_ui(
        title="Sequential Homological Motion: Setup + Reeb Playback",
        model_key="sequential_homological_motion",
        model_label="Sequential Homological Motion",
        motion_factory=SequentialHomologicalMotion,
