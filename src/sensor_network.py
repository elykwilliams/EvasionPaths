...
@dataclass
class AnnulusFenceSensors:
    sensors: list
    fence_groups: tuple[tuple[int, ...], ...]
...
class SensorNetwork:
    ...
    def __init__(
        ...
        self.domain = domain
        self.use_weighted_alpha = use_weighted_alpha
        self.fence_groups = tuple(tuple(int(v) for v in group) for group in (fence_groups or ()))
        self.excluded_fence_groups = tuple(int(v) for v in (excluded_fence_groups or ()))
    ...
    def point_radii(self):
        return np.array([s.radius for s in self], dtype=float)
...
def generate_fence_sensors(domain, radius, offset_distance=None):
    return [
        Sensor(point, (0, 0, 0), radius, True)
        for point in domain.fence(radius, offset_distance=offset_distance)
    ]


def generate_annulus_fence_sensors(domain, radius, offset_distance=None):
    from boundary_geometry import SquareAnnulusDomain

    if not isinstance(domain, SquareAnnulusDomain):
        raise TypeError("generate_annulus_fence_sensors() requires a SquareAnnulusDomain.")

    spacing = float(radius)
    dx = spacing * np.sin(np.pi / 3) if offset_distance is None else float(offset_distance)

    outer = domain.outer_half_side
    inner = domain.inner_half_side

    def square_loop(lo, hi):
        points = []
        tol = 1.0e-12
    ...
    outer_points = square_loop(-outer - dx, outer + dx)

    # Inner fence is offset into the hole and ordered clockwise so it tracks the
    # hole boundary with opposite orientation from the outer fence.
    inner_min = -inner + dx
    inner_max = inner - dx
    if inner_min >= inner_max:
        raise ValueError("Inner annulus fence offset is too large for the hole size.")
    inner_points = list(reversed(square_loop(inner_min, inner_max)))

    all_points = outer_points + inner_points
    sensors = [Sensor(np.array(point, dtype=float), np.zeros(2, dtype=float), radius, True) for point in all_points]
    outer_group = tuple(range(0, len(outer_points)))
    inner_group = tuple(range(len(outer_points), len(all_points)))
    return AnnulusFenceSensors(
        sensors=sensors,
        fence_groups=(outer_group, inner_group),
